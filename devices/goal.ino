...
unsigned long mainStartTime;                           /*<-- Timestamp for program start */
const unsigned long mainFinishTime = 30000;            /*<-- Program duration (30 seconds) */
...
const char* default_TOPIC_COORDINATES = "/TEF/device/g"; /*<-- MQTT topic for publishing coordinates */
...
void setup() {
  ...
  mainStartTime = millis();                          /* <-- Record program start time */
...
void loop() {
  ...
  bool mainState = true;                             /* <-- Control variable for main program state */
  ...
  if (millis() - mainStartTime >= mainFinishTime){   /* <-- Check if 30 seconds have elapsed */
    mainState = false;                               /* <-- Stop program execution */
  }
...
void gpsCommunication(){
  ...
  while(millis() - start < 5000 && !coordinatesPublished){
    ...
    if (gps.location.isUpdated() && gps.location.isValid()){
      ...
      String coord = lat + "," + lng;                /* <-- Combine coordinates into single string */
